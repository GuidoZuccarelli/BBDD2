...
public class SitesRestfulAPI {
	...
	public Response addCart(@PathParam("sitetoken") String sitetoken,
			@PathParam("userid") String userId) {
		try {
			return Response.status(201)
					.entity(this.service.addCart(sitetoken, userId)).build();
		} catch (NoSuchElementException e) {
			return Response.status(412).entity("Site token doesn't exists")
					.build();
		}
	}
