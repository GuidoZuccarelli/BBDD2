...
public class CartsRestfulAPI {
	...
	public Response addProduct(@PathParam("carttoken") String cartToken,
		...
		try {
			if (price != null) {
				if (price < 0)
					return Response.status(401)
							.entity("Price cannot be negative").build();
				else if (this.service.addProduct(cartToken, productId, price))
			...
			return Response.status(400)
					.entity("The price of the product is needed").build();
		} catch (NoSuchElementException e) {
			return Response.status(402).entity("Cart token doesn't exists")
					.build();
		} catch (TimeLimitExceededException g) {
			return Response.status(403).entity("Cart time limit exceed")
					.build();
		}
	}
	...
	@PUT
	@Path("/{carttoken}/products/{productid}")
	public Response updateProduct(@PathParam("carttoken") String cartToken,
			@PathParam("productid") String productId,
			@QueryParam("price") Double price,
	...
	public Response getCart(@PathParam("carttoken") String cartToken) {
		try {
			return Response.ok().entity(this.service.getCart(cartToken))
					.build();
		} catch (NoSuchElementException e) {
	...
	public Response getCart(@PathParam("carttoken") String cartToken,
		...
		try {
			return Response.ok()
					.entity(this.service.getProduct(cartToken, productId))
					.build();
		} catch (NoSuchElementException e) {
	...
	@DELETE
	@Path("/{carttoken}/products/{productid}")
	public Response deleteProduct(@PathParam("carttoken") String cartToken,
			@PathParam("productid") String productId) {
		try {
